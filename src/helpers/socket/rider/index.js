    ...
    if (!this.socket) {
      ...
      this.socket.on("requestAcceptedRider", res => {
        ...
        if (res.success) {
          ...
          if (userTripData) {
            userTripData.tripRequestStatus = Constants.AppCosntants.RideStatus.Accepted;
      ...
      this.socket.on("tripLocationUpdatedRider", res => {
        try {
          ...
          if (res.success) {
            ...
            if (tripRequestStatus === Constants.AppCosntants.RideStatus.Accepted && !isRecentAccepted) {
            ...
            if (tripRequestStatus === Constants.AppCosntants.RideStatus.EnRoute && !isRecentAccepted) {
      ...
      this.socket.on("requestCompletedRider", res => {
        ...
        if (res.success) {
          ...
          let userTripData = { ...riderTrip };
          if (userTripData) {
            userTripData.tripRequestStatus = Constants.AppCosntants.RideStatus.Completed;
            // console.log("USER trip data--------->", userTripData);
            dispatch({ type: Types.UPDATE_TRIP, payload: userTripData });
          }
      ...
      this.socket.on("requestEnrouted", res => {
        ...
        if (res.success) {
          ...
          let userTripData = { ...riderTrip };
          if (userTripData) {
            userTripData.tripRequestStatus = Constants.AppCosntants.RideStatus.EnRoute;
            // console.log("USER trip data--------->", userTripData);
            dispatch({ type: Types.UPDATE_TRIP, payload: userTripData });
