...
describe("<OtpInputs />", () => {
  ...
  test("rendering with error", () => {
    const testWrapper = renderer.create(
      <OtpInputs errorMessage="ExampleErrorMessage" />
    );
    const errorTextElement = testWrapper.root.findByProps({
      testID: "errorText"
    });

    expect(testWrapper).toMatchSnapshot();
    expect(errorTextElement._fiber.stateNode.props.children).toEqual(
      "ExampleErrorMessage"
    );
  });
